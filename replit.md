...
- Fixed LEQVIO field mapping bug: Corrected incorrect field assignments where patient names appeared under enrollment date and dates under copay ID number. Now properly maps subscriber→Patient ID, effective_from→Enrollment Date, subscriber_id→Copay ID Number, coverage_status→Group Number across all extraction methods (Epic screenshots, insurance cards, text extraction).
